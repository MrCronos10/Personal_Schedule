...
- **An incremental count of distinct Articles per Word, on `WordProgress`.** Found by ticket 09's round-wide review: `VocabularyLibrary.stubbornWords()` fetches every `WordLookup` ever recorded on each call, the same unbounded-growth shape ticket 02 deliberately avoided for `CleanSighting`. The real fix is a counter updated as each Lookup happens rather than recomputed from scratch, which is a schema and rule change deserving its own test-first ticket — not a patch inside a closing sweep. Not urgent at the scale of one student's year, but real, and worth doing before it isn't.
